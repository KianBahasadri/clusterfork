...
Standalone Python utilities in the repo root.
...
```bash
python auth_convert.py --from opencode path/to/auth.json   # → Codex format (stdout)
python auth_convert.py --from codex path/to/auth.json      # → OpenCode format (stdout)
python auth_convert.py --from auto path/to/auth.json       # auto-detect format
```
...
## enumerate_pioneer.py

Lists all models available on the Pioneer API with pricing, context windows, and probed reasoning effort tiers.

```bash
PIONEER_API_KEY=... python enumerate_pioneer.py [--json] [--include-deprecated] [--skip-probe] [--workers N] [--probe-workers N]
```
