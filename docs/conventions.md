...
- `~/.codex/config.toml`: updates top-level settings defined in `agents/codex.toml`, replaces `mcp_servers` and hook event tables, strips retired clusterfork keys (`notify`), and upserts `trusted_hash` for the clusterfork Stop bell (`hooks.state` otherwise stays); the model, approval settings, and the `[projects]` trust levels Codex writes itself stay untouched, and the installer aborts rather than write if anything else outside the repo template would change
