...
- `~/.codex/config.toml`: updates top-level settings defined in `agents/codex.toml` (such as `notify`) and replaces `mcp_servers` tables; the model, approval settings, and the `[projects]` trust levels Codex writes itself stay untouched, and the installer aborts rather than write if anything else outside the repo template would change
