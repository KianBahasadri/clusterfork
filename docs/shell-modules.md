...
Defaults, all overridable: `OCC_MODEL=deepseek-v4-flash`, `OCC_SONNET_MODEL=deepseek-v4-flash`, `OCC_SMALL_MODEL=minimax-m3`, `OCC_EFFORT=max`. The small slot stays on a cheaper model because it drives the background classifier and other trivial calls. Also honoured: `OPENCODE_GO_BASE_URL`, `OCC_MAX_CONTEXT_TOKENS`, `OCC_MAX_OUTPUT_TOKENS`, `OCC_OPENCODE_AUTH_FILE`, `OCC_MODELS_CACHE`, `OCC_GATEWAY_MODELS`, `OCC_MODEL_DISCOVERY`.
...
`occ` defaults to `--effort max` (overridable via `OCC_EFFORT` or by passing `--effort <level>` yourself). It uses the CLI flag rather than `CLAUDE_CODE_EFFORT_LEVEL` so `/effort` still works mid-session. On flash, graded effort separates at the top rung — see [Reasoning effort](opencode-go.md#reasoning-effort-works-on-some-models-some-clients).
