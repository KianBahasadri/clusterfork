...
`/effort` and `--effort` still appear under `occ`, but selecting a level does nothing — see [Reasoning effort](opencode-go.md#reasoning-effort-is-accepted-and-ignored). Adding `cl`'s `--effort xhigh` to the `occ` invocation would buy nothing.
