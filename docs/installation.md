...
| `agents/codex.toml`               | `~/.codex/config.toml`                   | Codex settings and MCP servers (merges top-level keys, replaces `mcp_servers` and hook events, drops retired `notify`, trusts Stop bell) |
...
- Updates top-level settings defined in `agents/codex.toml`, replaces `mcp_servers` and hook event tables, strips retired clusterfork keys (`notify`), and stamps `trusted_hash` for the Stop bell in `~/.codex/config.toml`; Codex's model, approval settings, `[projects]` trust levels, and other `hooks.state` entries are left alone
