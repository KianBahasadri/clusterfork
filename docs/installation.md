...
| `notify/`                         | `~/.config/clusterfork/notify/`          | Private ntfy Compose and route settings |
...
Each `✓` step line prints the label and destination; a parenthesized detail names what the step installs inside that destination — completion hooks (`Stop → clusterfork-notify`), MCP server names (read live from the source files), merged keys, shipped-disabled plugin names, helper script names, or skill-tree notes. A `+` line under a step is an extra destination installed by it (the statusline usage fetchers). Details are derived from the repo sources at run time, so they track the configs without edits to the installer.
