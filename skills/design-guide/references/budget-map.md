...
- Place the plot directly beneath the component section heading. Omit the dataset-title, date-range, and sample-data header. Use the dataset title for the plot's accessible name and a screen-reader-only dialog heading. Center the plane, cap its width at 720px, leave 16px clearance around it, and derive height from the same projection ratio rather than stretching the camera on mobile. Retain vertical scrolling and zoom.
...
- Use `--accent` for observed/current readings and `--derived` for forecasts. Apply these colors to the summary's current and forecast labels, dates, values, and units; forecast percentage and headroom are also derived. In the exact-data table, color each sample's timestamp, kind, available value, unit, and percentage according to its provenance. Keep reference limits, item names, headers, and missing-value dashes neutral. Preserve the separate caution/danger treatment of the budget's health status. Retain explicit `Current`, `Observed`, and `Forecast` wording so color is not the only distinction; do not add a legend.
- Size the summary dialog to at most 560px wide and expand it to at most 880px while exact data is visible, always keeping 12px viewport clearance. Use one header row for the wrapping service picker and the close control, separated by 8px; align the close control with the first picker row. Keep this header outside the scrolling body. Omit a separate title/close row and footer. Put `Show exact data` and `Export all data CSV` together immediately below the summary, with an 8px gap and wrapping when needed. Use 16px vertical margins around the summary's value list and let its values fill the available width, capped at 504px in the expanded view.
