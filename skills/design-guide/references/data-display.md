...
* Make every column sortable and put a native button inside every column header, including status and error rate. Support pointer click, `Enter`, and `Space` through native button behavior. Sort ordinary text alphabetically without case sensitivity; sort numeric values by their number, preserving decimals and ignoring thousands separators and display units.
