...
* Wrap a table in a full-width region with horizontal overflow and no outer border. Use a 6px radius and `--surface` background on the table region.
...
* Make every column sortable and put a native button inside every column header, including status, error rate, and action columns. Support pointer click, `Enter`, and `Space` through native button behavior. Sort ordinary text and action labels alphabetically without case sensitivity; sort numeric values by their number, preserving decimals and ignoring thousands separators and display units.
