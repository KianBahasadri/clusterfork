...
* On touch screens, let the tab panels follow horizontal finger movement directly across the tab strip or panel. Slide the adjacent panel in beside the current one with a 16px gap inside a clipped panel viewport; do not create horizontal page scrolling. Keep the selected panel's height during the drag, limit travel to one panel plus that gap, and apply resistance when pulling past the first or last tab. A neighboring panel shown only for the drag must be `inert` and `aria-hidden="true"`; restore `hidden` when it is no longer being previewed.
* On release, advance one tab for a left swipe or go back one for a right swipe with at least 48px travel and horizontal movement at least 1.5 times vertical movement. Settle the panels and viewport height over 220ms with `cubic-bezier(0.2, 0.8, 0.2, 1)`; short or canceled drags settle back to the selected tab. Commit selection once settling completes, suppress the click generated by a drag, and keep taps immediate. Use the same selection path for swipes, clicks, and keys so `aria-selected`, roving tab stops, and panel visibility stay synchronized. Under reduced motion, keep direct finger tracking but finish without a settling animation. A new interaction or viewport resize must leave a complete selection with no stranded transforms or preview panels.
* Preserve vertical scrolling and pinch-to-zoom. Do not change tabs for short drags, vertical gestures, multiple touches, canceled touches, text selection, or gestures originating in nested controls or horizontally scrolling content; offer an explicit opt-out for a component that owns its own gesture. Keep touch listeners passive except for the movement listener that prevents native handling after a horizontal intent is established. A swipe must not move keyboard focus from outside the tab group or scroll the page to its header.
