...
* Use a native `<input type="range">` for sliders and set explicit `min`, `max`, `step`, and initial `value` attributes. Pair each slider with a visible label and a live `<output>`; update the output on every `input` event and set `aria-valuetext` when the displayed value includes a unit or named step.
* Give a slider a fluid width capped at 320px, a 44px-high interaction box, a 4px `--line-strong` pill track, and an 18px circular `--control-selected` thumb. Fill the selected portion of the track with `--control-selected`; use `--control-selected-hover` for the fill and thumb while hovered. Render its right-aligned value in 12px `--mono` with tabular numbers.
* For a stepped slider, use the native `step` interval and render its finite values as evenly spaced 10px `--mono` labels beneath the track. Hide the repeated scale from assistive technology because the input already exposes its minimum, maximum, step, and current value.
* When a large numeric span makes direct pointer dragging too coarse, add precision scrubbing without replacing the native range input. During a captured pointer drag, use the normal horizontal rate within 24px of the drag origin, `0.25×` from 25–72px above or below it, `0.1×` from 73–144px, and `0.02×` beyond 144px. Apply the rate only to horizontal movement, preserve the value during vertical-only movement, and reset the rate when the drag ends. Set `touch-action: none` on that slider, emit `input` as its value changes and one `change` when the drag commits, and describe the vertical fine-scrub gesture and typed-value entry through `aria-describedby`.
* Show the active speed only while the pointer is captured. Hide it at rest and do not place it beside the value. Label the popup `Speed` followed by the multiplier (`Speed 1×`, `Speed 0.25×`, `Speed 0.1×`, `Speed 0.02×`). Render it 8px above the thumb: 13px `--mono` weight 600 with tabular numbers, `--ink-strong` on `--surface-raised`, no border, 6px radius, 4px vertical and 8px horizontal padding, and the large overlay shadow. Keep the popup centered on the thumb, clamp it 12px inside the visual viewport, and flip below the thumb when there is not enough space above. Treat the rate popup as non-interactive (`pointer-events: none`) and announce rate changes through a polite live region.
* Double-clicking the thumb, or pressing `Enter` while the slider is focused, opens a compact numeric field in that same popup so the user can type a value. Prefill the current numeric value without grouping or a unit suffix, move focus into the field, and select its contents. Size the field from the longer of the minimum or maximum value. While editing, enable pointer events on the popup, remove its `aria-hidden` state, and name the field from the slider's label. `Enter`, blur, or outside activation commits the parsed number, clamped to `min`/`max` and snapped to `step`; `Escape` or an unparseable entry cancels and restores the prior value. Emit `input` and `change` only when a commit changes the slider value. Closing restores focus to the slider without scrolling it out of place. Keep the native input keyboard-operable while the field is closed.
* Preserve the native slider keyboard behavior, including arrow, Page Up, Page Down, Home, and End keys. Pointer precision scrubbing is an enhancement to that keyboard-operable input, not a pointer-only replacement.
* Set the native `disabled` attribute on disabled checkboxes, radios, switches, and sliders. Preserve whether a binary control is checked or unchecked, apply opacity 0.45 and `cursor: not-allowed` to the visible control, label, and slider value, suppress every hover treatment, and prevent changes. In a complete selection-control reference, show a disabled example of all four control types and include both checked and unchecked disabled states across the checkbox, radio, and switch set.

### Slider Source and Reuse

The [selection examples](../assets/component-reference/components/forms-selection.html) contain ordinary, stepped, precision, and disabled sliders. Their `.range-control` markup and range styles in `components/forms.css` also support the budget-map controls. Set `data-range-output` to the output element's ID; optional `data-range-suffix`, `data-range-aria-unit`, and `data-range-grouped` control visible and spoken formatting. Add `.range-slider-precision` and point `data-range-scrub-status` to a polite live region for precision entry.

The two slider initializers in `components/forms.js` update `--range-progress`, synchronize outputs, and attach precision interaction when the script loads. For standalone use, extract those initializers with the range styles: the complete file also binds the catalog's fixed search and email-form IDs and cannot be loaded unchanged on a page containing only sliders. The chart explorer and realtime simulation toolbar retain their own compact range controls and controllers.
