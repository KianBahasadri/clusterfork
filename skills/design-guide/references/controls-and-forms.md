...
* When a large numeric span makes direct pointer dragging too coarse, add precision scrubbing without replacing the native range input. During a captured pointer drag, use the normal horizontal rate within 24px of the drag origin, `0.25×` from 25–72px above or below it, `0.1×` from 73–144px, and `0.02×` beyond 144px. Apply the rate only to horizontal movement, show the active rate beside the value, preserve the value during vertical-only movement, and reset the rate when the drag ends. Set `touch-action: none` on that slider, emit `input` as its value changes and one `change` when the drag commits, and describe the vertical fine-scrub gesture through `aria-describedby`.
