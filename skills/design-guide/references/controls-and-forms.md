...
* Double-clicking the thumb, or pressing `Enter` while the slider is focused, opens a compact numeric field in that same popup so the user can type a value. Prefill the current numeric value without grouping or a unit suffix, move focus into the field, and select its contents. Size the field from the longer of the minimum or maximum value. `Enter` or outside activation commits the parsed number, clamped to `min`/`max` and snapped to `step`; `Escape` or an unparseable entry cancels and restores the prior value. Closing restores focus to the slider without scrolling it out of place. Keep the native input keyboard-operable while the field is closed.
