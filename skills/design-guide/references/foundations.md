...
* Give every result a working page-wide shortcut. Use `Alt+J` to open Spotlight directly in destination mode, `Alt+Shift+M` for Markdown export, `Alt+Shift+P` for PDF export, `Alt+Shift+T` for theme toggle, and `Alt+/` for the shortcut popup. Jump to sections 01–10 with `Alt+1` through `Alt+0`, then sections 11–16 with `Alt+Shift+1` through `Alt+Shift+6`.
