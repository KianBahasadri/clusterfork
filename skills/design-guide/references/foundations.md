...
* Give every result a working page-wide shortcut. Use `Alt+J` to open Spotlight directly in destination mode, `Alt+Shift+M` for Markdown export, `Alt+Shift+P` for PDF export, `Alt+Shift+T` for theme toggle, and `Alt+/` for the shortcut popup. Jump to sections 01–10 with `Alt+1` through `Alt+0`, then sections 11–14 with `Alt+Shift+1` through `Alt+Shift+4`.
...
* Share the current section between navigation highlighting and keyboard traversal. Prefer the explicitly selected destination while its heading remains visible; otherwise use the nearest section at or above the 120px reading offset. Keep sections that share a row individually addressable, including when the whole dashboard fits without scrolling. Refresh selection on anchor changes as well as scrolling and resizing.
...
Dashboard page composition is specified in [Dashboard Composition](dashboard-composition.md#layout).

* For the component catalog on desktop, cap content at 1240px, center it, use 32px top, 24px horizontal, and 80px bottom padding, and use a 240px navigation column plus one flexible content column separated by 40px.
* Collapse to one column at 900px and below and replace the inline component index with a fixed top-left Lucide `Menu` button named `Open table of contents`. Use a transparent 44×44px target with an 18px icon, aligned vertically with the search and theme controls: 16px top and 20px left above 600px, 8px top and left at 600px and below. Reserve 80px above content between 601px and 900px. Hide this launcher when the desktop index is visible.
* Open the mobile table of contents in a labelled native modal dialog anchored to the left edge. Use the full dynamic viewport height, width `min(320px, 100vw - 48px)`, `--surface-raised`, a 1px `--line-strong` right border, square corners, the large overlay shadow, and a 65% black backdrop. Keep an 18px title and 44×44px Lucide `X` close control above an independently scrolling list with 44px minimum link targets. The drawer may enter from the left over 220ms; omit this animation under reduced motion.
* Reuse one navigation list between desktop and mobile. Mark the current section with `aria-current="location"` and the active-row treatment. Opening the drawer focuses and reveals the current link, keeps focus inside, locks background scrolling, and updates the launcher's `aria-expanded`. Close on `Escape`, the close control, or a backdrop tap and return focus without changing page scroll. Choosing a section closes the drawer, updates the URL fragment, scrolls to the section, and focuses its heading. On returning to desktop width, close the drawer, release the scroll lock, and restore the index to its column.
* At 600px and below, reserve 68px of top space for utilities. In the component catalog, use 16px horizontal page padding and reduce static example wrappers to 16px vertical padding with no extra horizontal inset.
